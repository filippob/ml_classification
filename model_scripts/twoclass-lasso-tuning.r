...
outdir = "results/twoclass/lasso"
nproc <- 4
method_cv <- "repeatedcv"
k_folds <- 5
nrepeats_cv <- 3
nlevels = 20 ## levels of hyperparameters to test
target_var = "Label"
...
## log file
fname = paste(tmstmp, ".twoclass-lasso-tuning.r.log", sep="")
...
lasso_hyperparams <- parameters(
  penalty(range = c(-5, 0))
)
lasso_grid <- grid_regular(lasso_hyperparams, levels = nlevels)
